...
| 4 ✅ | 78 | Playback + cache: `CacheStoreTests` (25 sync-API tests), `CacheStoreStressTests` (concurrent readers/writer under `@Suite(.serialized)`), `CachingResourceLoaderTests`, `CacheAuthFailureTests` (401/403 → `.taAuthUnauthorized`), `VideoCachePreloaderTests` (incl. baseline + memory pressure + HEAD-probe success and HEAD-probe 401), `PlayerSessionCoordinatorTests` (interruption/route/mediaServicesReset), `NowPlayingControllerTests`, `ObserverBagTests`, `StreamingSessionTests` (URLSession leak fix: natural completion, consumer drop, network error) |
