...
struct VideoMapperTests {
    ...
    @Test func minimalValidDTO_returnsVideo() {
        let dto = VideoDTO(
            ...
            stats: nil, vidType: nil, category: nil, tags: nil, streams: nil
        )
    ...
    @Test func relativeThumbUrl_prependsServerURL() {
        let dto = VideoDTO(
            ...
            player: nil, stats: nil, vidType: nil, category: nil, tags: nil, streams: nil
        )
        let video = VideoMapper.map(dto, serverURL: serverURL)
    ...
    @Test func absoluteThumbUrl_keptAsIs() {
        let dto = VideoDTO(
            ...
            category: nil, tags: nil, streams: nil
        )
        let video = VideoMapper.map(dto, serverURL: serverURL)
