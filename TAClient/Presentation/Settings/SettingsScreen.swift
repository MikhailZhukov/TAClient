...
/// Thin wrapper view that owns a `SettingsViewModel` via `@State` so the VM is
...
/// `TAClientApp` would call `container.makeSettingsViewModel()` inline,
...
struct SettingsScreen: View {
    // INVARIANT: SettingsViewModel.init must remain side-effect-free.
